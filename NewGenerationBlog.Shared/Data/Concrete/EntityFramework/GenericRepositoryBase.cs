...
namespace NewGenerationBlog.Shared.Data.Concrete.EntityFramework
{
    public class GenericRepositoryBase<T> : IGenericRepositoryBase<T> where T : class
    {
        ...
        public async Task<IQueryable<T>> GetAllAsync(Func<IQueryable<T>, IIncludableQueryable<T, object>> include)
        {
        ...
        {
            await Task.Run(() => { _dataBaseContext.Set<T>().Update(entity); return Task.CompletedTask; });
            return entity;
        }
        ...
        {
            await Task.Run(() => {  _dataBaseContext.Set<T>().Remove(entity); });
        }
    }
}